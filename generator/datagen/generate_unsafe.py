...
from Prompts import format_for_create
import warnings
import random
...
def parse_arguments():
    ...
    parser.add_argument('--model', type=str, default='cognitivecomputations/dolphin-2.9.4-llama3.1-8b',
                        help='Path or identifier of the model to use.')
    parser.add_argument('--tensor_parallel_size', type=int, default=1,
                        help='Number of GPUs to use for model parallelism in vLLM.')
    parser.add_argument('--pipeline_parallel_size', type=int, default=1,
                        help='Number of pipeline parallel stages in vLLM (optional).')
...
def main():
    ...
    logging.info("Filtering data based on moderation_flag...")
    data = data.filter(lambda example: example["moderation_flag"] == 1)
    ...
    prompts_all = [
        format_for_create(example["prompt"], tokenizer, language=language)
    ...
    try:
        for i in tqdm(range(0, len(prompts_all), batch_size)):
            ...
            results.extend([
                [r.text.replace("</s>", "").lstrip() for r in result.outputs] 
    ...
    output_file = output_dir / f"{language}_unsafe_prompts_{output_name}.json"
